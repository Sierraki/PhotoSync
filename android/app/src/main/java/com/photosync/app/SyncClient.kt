...
class SyncClient(private val context: Context) {
    ...
    private val client: OkHttpClient by lazy {
        val builder = OkHttpClient.Builder()
            ...
            .writeTimeout(300, TimeUnit.SECONDS)  // 大文件增加超时
    ...
    private fun registerConnection() {
        try {
            val connType = when (connectionMode) {
                ConnectionMode.WIFI -> "wifi"
                ConnectionMode.USB -> "adb"
            }
            val requestBody = MultipartBody.Builder()
                .setType(MultipartBody.FORM)
                .addFormDataPart("device", android.os.Build.MODEL)
                .addFormDataPart("connection_type", connType)
                .build()

            val request = Request.Builder()
                .url("$baseUrl/api/phone/register")
                .post(requestBody)
                .build()
    ...
    fun unregisterConnection() {
        try {
            val request = Request.Builder()
                .url("$baseUrl/api/phone/disconnect")
    ...
    /**
     * 检查 PC 端是否有同步请求，并返回请求的同步模式
     */
    fun checkSyncRequest(): SyncRequestState {
        return try {
            val request = Request.Builder()
                .url("$baseUrl/api/wifi/check-request")
                .get()
                .build()
            val response = client.newCall(request).execute()
            if (response.isSuccessful) {
                ...
                val requestSync = (data["request_sync"] as? Boolean) == true
                val mode = (data["sync_mode"] as? String)?.lowercase(Locale.getDefault()) ?: "incremental"
                val normalizedMode = if (mode == "full") "full" else "incremental"
                SyncRequestState(requestSync, normalizedMode)
            } else {
                SyncRequestState(false, "incremental")
            }
        } catch (e: Exception) {
            SyncRequestState(false, "incremental")
        }
     ...
     * 检查 PC 端是否请求停止同步
     */
    fun checkStopRequest(): Boolean {
        return try {
            val request = Request.Builder()
                .url("$baseUrl/api/wifi/check-stop-request")
                .get()
                .build()
            val response = client.newCall(request).execute()
            if (response.isSuccessful) {
                val body = response.body?.string() ?: "{}"
                val data = gson.fromJson(body, Map::class.java)
                (data["request_stop"] as? Boolean) == true
            } else {
                false
            }
        } catch (e: Exception) {
     ...
     * 获取服务器端 WiFi 同步状态快照
     */
    fun getWifiSyncStatus(): Result<Map<String, Any>> {
        return try {
            val request = Request.Builder()
                .url("$baseUrl/api/wifi/status")
                .get()
                .build()
            val response = client.newCall(request).execute()
            if (response.isSuccessful) {
                ...
                Result.failure(IOException("状态获取失败: ${response.code}"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
     ...
     * 向服务器发送停止/取消同步请求
     */
    fun requestStopSync(): Result<String> {
        return try {
            val request = Request.Builder()
                .url("$baseUrl/api/wifi/request-stop")
                .post("".toRequestBody("text/plain".toMediaType()))
                .build()
            val response = client.newCall(request).execute()
            if (response.isSuccessful) {
                val body = response.body?.string() ?: "{}"
                val data = gson.fromJson(body, Map::class.java)
                val message = (data["message"] as? String) ?: "已发送停止请求"
                Result.success(message)
            } else {
                Result.failure(IOException("停止请求失败: ${response.code}"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    /**
     * 根据手机端清单批量比对电脑数据库（album + filename + size）
     */
    fun checkManifest(items: List<Map<String, Any>>): Result<Map<String, Boolean>> {
        return try {
            val jsonBody = gson.toJson(items)
            val requestBody = jsonBody.toRequestBody("application/json".toMediaType())
            val request = Request.Builder()
                .url("$baseUrl/api/check_manifest")
                .post(requestBody)
                .build()
            val response = client.newCall(request).execute()
            if (response.isSuccessful) {
                ...
                Result.failure(IOException("清单比对失败: ${response.code}"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
     ...
     * 通知服务器扫描进度（扫描阶段）
     */
    fun notifyScanProgress(deviceName: String, scanned: Int, total: Int, syncMode: String) {
        try {
            val normalizedMode = if (syncMode.lowercase(Locale.getDefault()) == "full") {
                "full"
            } else {
                "incremental"
            }
            val requestBody = MultipartBody.Builder()
                ...
                .addFormDataPart("sync_mode", normalizedMode)
                .build()
     ...
     * 通知服务器开始 WiFi 同步
     */
    fun notifySyncStart(deviceName: String, phoneTotal: Int, needSync: Int, syncMode: String) {
        try {
            val connType = when (connectionMode) {
                ConnectionMode.WIFI -> "wifi"
                ConnectionMode.USB -> "adb"
    ...
    fun notifySyncProgress(
        ...
        try {
            val requestBody = MultipartBody.Builder()
                ...
                .addFormDataPart("bytes_sent", bytesSent.toString())
                .addFormDataPart("speed_mbps", speedMbps.toString())
                .addFormDataPart("eta_seconds", etaSeconds.toString())
                .build()
    ...
    /**
     * 上报手机端日志到服务器，供网页端显示与手机一致的日志内容
     */
    fun notifySyncLog(message: String) {
        try {
